...
import { useEvidenceSnapshotStore } from '../stores/evidence-review-snapshot';
import { registerSensorEvidence } from '../api/sensor-evidence';
import { transitionExcursionEvent } from '../api/excursion-event';
...
export default function ExcursionEventPage() {
  ...
  const snapshotStore = useEvidenceSnapshotStore();
  ...
  const refresh = async () => { await Promise.all([excursions.load('excursions'), dispositions.load('dispositions'), windows.load('windows'), evidenceStore.load(), snapshotStore.load()]); };
  useEffect(() => { void refresh(); }, [excursions.load, dispositions.load, windows.load, evidenceStore.load, snapshotStore.load]);
  ...
  const snapshot = selected ? snapshotStore.items.find((item) => item.excursionCode === selected.code) || null : null;
  ...
  const createExcursion = async () => {
    ...
    // Each registered artifact needs a distinct SHA-256: a frozen review snapshot digest can never
    // be reused by another excursion, so derive a unique 64-hex digest from this registration.
    const digest = Array.from({ length: 64 }, (_, index) => ((suffix.charCodeAt(index % suffix.length) + index * 7) % 16).toString(16)).join('');
  ...
  const transition = async () => {
    if (!pending) return;
    const reason = pending.state === 'in_review' ? '质量复核员接收偏差并核对传感器曲线'
      : pending.state === 'decided' ? '传感器证据完整，偏差影响评估完成' : '关联处置决定已完成';
    try {
      await transitionExcursionEvent(pending.item.id, pending.state, pending.item.version, reason, pending.item.sensorEvidence || pending.item.evidence);
      await refresh();
    } catch (error) {
      // A snapshot conflict (missing evidence / container mismatch / reused digest) keeps the
      // excursion in_review; reload so the persisted block reason is displayed after refresh.
      await refresh();
    } finally {
      setPending(null);
    }
...
<DecisionPanel decision={decision} compact />{canReview && nextExcursionState(selected) && <Button variant="contained" startIcon={<TaskAltOutlinedIcon />} onClick={() => setPending({ item: selected, state: nextExcursionState(selected) })}>{selected.status === 'open' ? '接收复核' : selected.status === 'in_review' ? '完成影响评估' : '关闭事件'}</Button>}</> : <div className="empty">选择一个偏差事件</div>}</aside></section>
    ...
    <ConfirmDialog open={Boolean(pending)} title="确认偏差状态迁移" onCancel={() => setPending(null)} onConfirm={() => { void transition().catch(() => undefined); }}><p>偏差不能跳过复核；形成影响评估前系统会冻结一条已登记证据的复核快照（证据编号、SHA-256、容器编码）。证据缺失、容器不一致或摘要重复时保持待复核并返回冲突编号。</p><strong>{pending?.item.status} → {pending?.state}</strong></ConfirmDialog>
  </main>;
}
